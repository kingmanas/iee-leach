...
// the message length of the advertisement that there is a cluster head default
// is 16
...
int main(int argc, char *argv[]) {
  ...
  printf("The LEACH simulation was able to remain viable for %d rounds\n",
         rounds_LEACH / TRIALS);
  printf(
      "The direct transmission simulation was able to remain viable for %d "
      "rounds\n",
...
int runLeachSimulation(const struct sensor network[]) {
  ...
  vector<double> dir;
  // old :: network_LEACH = (struct sensor *) malloc(NUM_NODES * sizeof(struct
  // sensor));
  ...
  while (averageEnergy(network_LEACH) > .10) {
    ...
    for (i = 0; i < NUM_NODES; i++) {
      if (network_LEACH[i].head == -1) {
        network_LEACH[i].bCurrent -=
            computeEnergyTransmit(LEACH_AD_DISTANCE, LEACH_AD_MESSAGE);
      } else {
      ...
      if ((network_LEACH[i].head != -1) && network_LEACH[i].head != DEAD_NODE) {
        ...
        for (k = 0; k < NUM_NODES; k++) {
          if (network_LEACH[k].head == -1 && closest != -1) {
            distance_X_old =
                network_LEACH[i].xLoc - network_LEACH[closest].xLoc;
            distance_Y_old =
                network_LEACH[i].yLoc - network_LEACH[closest].yLoc;
            distance_old =
                sqrt(pow(distance_X_old, 2) + pow(distance_Y_old, 2));
            distance_X_new = network_LEACH[i].xLoc - network_LEACH[k].xLoc;
            distance_Y_new = network_LEACH[i].yLoc - network_LEACH[k].yLoc;
            distance_new =
                sqrt(pow(distance_X_new, 2) + pow(distance_Y_new, 2));
            if (distance_new < distance_old) closest = k;
    ...
    for (i = 0; i < NUM_NODES; i++) {
      ...
      if (network_LEACH[i].head != -1 && network_LEACH[i].head != DEAD_NODE) {
        distance_X_new =
            network_LEACH[i].xLoc - network_LEACH[network_LEACH[i].head].xLoc;
        distance_Y_new =
            network_LEACH[i].yLoc - network_LEACH[network_LEACH[i].head].yLoc;
    ...
    for (i = 0; i <= NUM_NODES; i++) {
      if (network_LEACH[i].head == -1) {
        ...
        distance_new = sqrt(pow(distance_X_new, 2) + pow(distance_Y_new, 2));
        double energy_enough =
            network_LEACH[i].bCurrent -
            computeEnergyTransmit(
                distance_new,
                (MESSAGE_LENGTH * (network_LEACH[i].cluster_members + 1)));

        //            network_LEACH[i].bCurrent -=
        //            computeEnergyTransmit(distance_new,(MESSAGE_LENGTH *
        //            (network_LEACH[i].cluster_members+1)));
        ...
        if (energy_enough > 0.0) {
          network_LEACH[i].bCurrent -= computeEnergyTransmit(
              distance_new,
              (MESSAGE_LENGTH * (network_LEACH[i].cluster_members + 1)));
          bits_transmitted +=
              (MESSAGE_LENGTH * (network_LEACH[i].cluster_members + 1));
          ...
          while (power == FALSE) {
            if ((network_LEACH[i].bCurrent -
                 computeEnergyTransmit(
                     distance_new, (MESSAGE_LENGTH * temp_cluster_members))) >
                0) {
              bits_transmitted += (MESSAGE_LENGTH * temp_cluster_members);
              power = TRUE;
            } else
              temp_cluster_members--;
            if (temp_cluster_members == 0) power == TRUE;
          }
        }
      }
    }
    ...
    if (round == 1000)
      cout << "network_pwr: " << averageEnergy(network_LEACH) << endl;
    round += 1;
    j = round;
  }
  ofstream d_stats;
  ...
  for (unsigned int i = 0; i < dir.size(); i++)
    d_stats << dir[i] << endl;  // d_stats << dir[i] << endl ;
  // cout << dir.size() << endl;

  //}
  // cout << "LEACH: " << round << endl;
  // free(network_LEACH);
  // printf("The average energy of the network remained above 10%% for %d
  // tranmission periods\n", round);
...
int runDirectSimulation(const struct sensor network[]) {
  ...
  */
    ...
    for (i = 0; i < NUM_NODES; i++) {
      ...
      if (network_DIRECT[i].bCurrent > 0) {
        ...
        if (power_consumption <= network_DIRECT[i].bCurrent) {
          ...
          failed_transmission++;
          if (flag) {
            // cout << "power_consumption: " << power_consumption << "
            // network_DIRECT[i].bCurrent: " << network_DIRECT[i].bCurrent <<
            // endl; cout << "Directdead_node's round: " << round << endl;
            flag = 0;
          }
        }
      }
      if (network_DIRECT[i].bCurrent < 0) network_DIRECT[i].bCurrent = 0;
    }

    // if(round == 1000 ) cout  << "network_pwr: " <<
    // averageEnergy(network_DIRECT) << endl;
  ...
  for (unsigned int i = 0; i < dir.size(); i++) stats << dir[i] << endl;
...
int runDirectSimulationNPP(const struct sensor network[]) {
  ...
  for (j = 0; j <= TOTAL_ROUNDS; j++) {
    for (i = 0; i < NUM_NODES; i++) {
      ...
      if (network_DIRECT_NPP[i].bCurrent > 0) {
        network_DIRECT_NPP[i].pAverage =
            ((network_DIRECT_NPP[i].bPower - network_DIRECT_NPP[i].bCurrent) /
             network_DIRECT_NPP[i].lPeriods);
      }
    }
    if (nodes_transmitting == 0)
...
struct sensor *loadConfiguration(char *filename) {
  ...
  while (fgets(buf, 80, fp) != NULL) {
    ...
    if (cut[0] == '#') {
      ...
      // into the copy variable from the iterator i
      strcpy(copy, buf + i);
      if (axis == 1) NETWORK_X = atoi(copy);
      if (axis == 0) NETWORK_Y = atoi(copy);
...
int runmodification(const struct sensor network[]) {
  ...
  while (averageEnergy(network_LEACH) > .10) {
    ...
    // if(round == 1000 ) cout  << "network_pwr: " <<
    // averageEnergy(network_LEACH) << endl;
    round += 1;
    j = round;
